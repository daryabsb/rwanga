...
| **Removal condition** | (a) `Rga.ScriptMetrics` module exists with `wordCount` + `currentBlockType` fields (plus reserved-but-not-yet-populated `dialogueWords`, `actionWords`, `sceneCount`, `estimatedRuntime`). (b) `Rga.Shell.StatusBar` reads both values from `Rga.ScriptMetrics.get()` instead of `Rga.ScriptSession.get()`. (c) `Rga.Shell.Outline` reads `wordCount` from `Rga.ScriptMetrics.get()` instead of `Rga.ScriptSession.get()`. (d) Both fields are removed from `Rga.ScriptSession`'s snapshot shape; `Rga.ScriptSession.get()` returns exactly the seven writer-context fields (`activeScript`, `currentScene`, `currentPage`, `currentView`, `currentSelection`, `openPanels`, `activePanel`). (e) `script-session.test.js` shape assertions updated to the seven writer-context fields; integration test's `'wordCount' in snap` / `'currentBlockType' in snap` assertions moved to a new `script-metrics.test.js`. When all five are true, the misplacement is resolved and this entry moves to RESOLVED. |
