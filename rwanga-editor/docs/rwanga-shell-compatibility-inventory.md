...
| **Removal condition** | **(a) DONE in Slice 9 §A:** `Rga.Shell.StudioPanel` exists (`renderer/js/shell/studio-panel.js`) with the existing tab surfaces preserved. The Notes / Flags / Problems / Breakdown DOM IDs are unchanged — StudioPanel takes over the WIRING but leaves the DOM where engine plugins expect it. **(b) DONE in Slice 9 §A:** engine-plugin DOM target IDs are preserved exactly (no changes to `#annotation-notes-list`, `#revision-flags-list`, etc.). **(c) PARTIALLY OPEN:** `Rga.BottomPanel.init()` bootstrap call remains in `index.html` (now routes through the BottomPanel shim → `Rga.Shell.StudioPanel.init()`). The bootstrap can be replaced with a direct `Rga.Shell.StudioPanel.init()` call when the BottomPanel shim is removable (gated on `editor/*` / `doc-types/*` touchability). **(d) DONE in Slice 9 §A:** menu/command bindings now route through the BottomPanel shim, which delegates to StudioPanel — no binding has a direct `Rga.BottomPanel.toggleCollapse` implementation; all paths go through StudioPanel.<br>**Status RESOLVED-WITH-SHIM:** StudioPanel is the SSOT and owner; the `Rga.BottomPanel` shim (4 methods, ~20 LOC) remains in `app-shell.js` because two engine plugins (`annotations.js`, `revision-flags.js`) call `Rga.BottomPanel.switchTo(...)` directly. When `editor/*` / `doc-types/*` become touchable, those engine calls migrate to `Rga.Shell.StudioPanel.switchTo(...)` and the shim deletes. G12 drift guard (Slice 9 §B) pins the shim's shape so it cannot re-grow into an implementation. |
...
| **Removal condition** | **(a) DONE in Slice 5 §A:** `Rga.ScriptMetrics` module exists with `wordCount` + `currentBlockType` fields plus reserved fields. **(b) DONE in Slice 5 §A:** `Rga.Shell.StatusBar` reads both values from `Rga.ScriptMetrics.get()`. **(c) DONE in Slice 7 §A:** `Rga.Shell.Outline` does not read `wordCount` from `Rga.ScriptSession.get()` (investigation confirmed it reads directly from `Rga.Nav.getOutline().statistics.words`, never from ScriptSession). **(d) DONE in Slice 7 §A:** Both fields removed from `Rga.ScriptSession`'s snapshot shape; `EMPTY_SNAPSHOT`, `_recompute()`, `_clone()`, `_snapshotEquals()` all stripped. **(e) DONE in Slice 7 §A:** Shape assertions migrated — `script-session.test.js` Slice-2-era wordCount/currentBlockType tests deleted; new "Slice 7 §A shape lock" test asserts the 7-field-only invariant; integration test's `'wordCount' in snap` assertion inverted to `false`. Plus drift guard G8 enforces at CI. **Entry RESOLVED.** |
