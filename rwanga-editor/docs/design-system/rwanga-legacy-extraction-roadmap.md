...
| **Sidebar visibility / active panel** | `Rga.Shell.Sidebar` + `Rga.Shell.Layout.sidebar` | `Rga.Sidebar` in `app-shell.js:158` — 5-LOC no-op shim | `Rga.Sidebar.switchTo('tags')` called by engine plugin `tags.js:206` (off-limits) is a no-op | Slice when `editor/*` / `doc-types/*` becomes touchable; the call should migrate to `Rga.Shell.Sidebar.activate(panelId)` once the Characters panel exists (planned Slice 4+) | LOW — shim is pure no-op; engine call has no behavioural side effect today | OPEN — shim retained per Compatibility Inventory entry #2 |
| **StatusBar render + segments** | `Rga.Shell.StatusBar` (`renderer/js/shell/status-bar.js`) | Legacy `Rga.StatusBar` was retired in Slice 2 of the original shell migration; no shim remains in `app-shell.js` | None | n/a (resolved) | LOW | RESOLVED (Compatibility Inventory entry #1) |
| **BottomPanel visibility + active tab + persistence** | `Rga.Shell.Layout.studioPanel` (SSOT) + `Rga.BottomPanel` (public mutator API) | Same — `Rga.BottomPanel` is the legacy public API; Slice 1 routed all writes through it; Slice 2 consolidated Cmd+\` through the same mutator | `Rga.BottomPanel` (the legacy API) is the documented mutator. Engine plugins `annotations.js` and `revision-flags.js` (off-limits) call `Rga.BottomPanel.switchTo(...)` — preserved by design | Slice 4+ when `Rga.Shell.StudioPanel` is introduced per Compatibility Inventory entry #5 | MEDIUM — engine plugins call the public API; the StudioPanel migration must preserve the `switchTo(tabName)` signature | OPEN |
