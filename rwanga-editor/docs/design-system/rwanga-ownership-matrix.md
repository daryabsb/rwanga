...
| **Session boundary** (which session-side fields belong to which owner) | `Rga.SessionBoundary` (`renderer/js/shell/session-boundary.js`) — canonical manifest declaring 4 owners (ScriptSession / ScriptMetrics / ViewManager / WorkspaceState) and their fields. Pure data + helpers; no runtime side effects. **RESOLVED (Slice 7 §A + §B).** | Same — SessionBoundary is the doctrine module | None — the manifest IS the source of truth | n/a (resolved) | `Rga.SessionBoundary._MANIFEST` is the in-code SSOT; `tests/unit/shell/ownership-drift-guards.test.js` mirrors it and the G8 / G9 / G10 guards enforce CI-time compliance |
