...
// relative to the viewport, plus whether the submenu was flipped.
async function rightClickAt(page, x, y) {
  return page.evaluate(({ x, y }) => {
    ...
    return {
      hidden: el.hidden,
      flipSubmenu: el.classList.contains('ctx-flip-submenu'),
...
test('normal middle-of-line right-click places the menu at the click point, no flip', async () => {
  ...
  try {
    ...
    expect(r.flipSubmenu).toBe(false);
    // Click point is honored where there is room.
    expect(Math.round(r.left)).toBe(420);
    expect(Math.round(r.top)).toBe(320);
    await page.screenshot({ path: shotPath('01-middle.png') });
  } finally {
    await teardown(app, userDataDir);
  }
});

test('right-edge right-click keeps the menu inside the viewport and flips the submenu', async () => {
  ...
  try {
    ...
    assertInsideViewport(r);
    expect(r.flipSubmenu).toBe(true);
    await page.screenshot({ path: shotPath('02-right-edge.png') });
  } finally {
    await teardown(app, userDataDir);
  }
});
...
test('bottom-right corner right-click keeps the whole menu inside the viewport', async () => {
  ...
  try {
    ...
    assertInsideViewport(r);
    expect(r.flipSubmenu).toBe(true);
    await page.screenshot({ path: shotPath('04-corner.png') });
  } finally {
    await teardown(app, userDataDir);
  }
});
