...
| GAP-2-5 | S2.4F | **"Legal" paper size was unreachable — and it was never a dead entry.** Raised because S2.4F's honest dropdown stopped advertising Legal. **The orchestrator initially mis-read it as a dead constant and removed it; three unit tests went red and corrected that**: the layout/pagination engine has ALWAYS supported Legal end to end — dimensions in `Constants.PAPER_SIZES`, resolution in `LayoutProfile`, and `linesPerPage` asserted in `tests/unit/export/pdf-export.test.js` + `tests/unit/framework/layout-profile.test.js`. **Only the registry's `options` list omitted it**, so Settings made a working capability unreachable — the same control↔registry mismatch class as GAP-2-3, in its other direction. **RESOLVED 2026-07-29 (user: "it must stay, but in the settings"):** `'legal'` added to `pageSetup.paperSize` options + labels. Proven end to end, not asserted — `page-setup-paper-size.spec.js` "US Legal is selectable and really applies" measures a real **1344px (14in)** page. Note for the record: Legal is **not** the screenplay industry size (US Letter is; A4 elsewhere, and A4 is what this region uses) — it is simply supported and no longer hidden. `custom` remains registry-legal with no dimensions behind it, so the dialog still filters it out. | **CLOSED** |
| GAP-3-6 | S3.4F | **`Ctrl+Shift+T` is claimed by both `kb.toggleTheme` and the legacy shim's "tag as" binding** — structurally the identical defect S3.4F just fixed three times over, found by the new duplicate-defaults guard's own logic. The S3.3 console audit had classified it "benign", so it was never named in a ledger row and S3.4F's scope did not cover it (§0.2 scope freeze — reported, not touched). It is not benign in principle: the registry is last-wins, so one of the two silently loses. Small fix; fold it into the next shortcut/settings slice and let the standing guard prove it. | OPEN — small |
...
- **⚠ CURRENT baseline (2026-07-29, after S3.4F): `1942 tests · 1941 pass · 0 fail · 1 skip`.**
  The count moved 1936 → 1942 because S3.4F added the 6-test duplicate-shortcut-defaults guard.
  **Any red is a regression, and any count below 1942 means tests went missing.** History of the
  count: 1936 (S0.1/S1.5) → 1942 (S3.4F, +6 guard). Update this line whenever a slice adds tests.
...
- [x] **Step 1 — the guard test FIRST** (superpowers:test-driven-development), and it is the point of
      ...
      **Done:** `rwanga-editor/tests/unit/shell/kb-shortcut-default-collisions.test.js`, 6 tests.
      Confirmed RED on all three collisions before any source change (exact failure messages in
      `docs/plans/evidence/S3.4F-shortcut-collisions.md`).
- [x] **Step 2 — Save As keeps `Ctrl+Shift+S`** (the ruling). Give **Scene Navigator** a new default
      that collides with nothing; state the chosen key and why in the evidence file.
      **Done:** `kb.sceneNavigator` → `Ctrl+Shift+1` (matches the shell's already-real hardcoded
      binding — Settings honesty, not a new key). `kb.saveAs` unchanged.
- [x] **Step 3 — settle the two lesser collisions** found in the same audit: `Ctrl+Shift+E`
      ...
      **Done:** `kb.exportPdf` → `Ctrl+Alt+E`; search panel → `Ctrl+Shift+2`. Also fixed a stale
      `displayAccelerator: 'Ctrl+Shift+E'` menu label in `index.html` (`file.exportPdf`) that would
      have started lying the moment the default moved.
- [x] **Step 4 — prove it end to end:** Save As actually fires on `Ctrl+Shift+S` in a running app, and
      ...
      **Done:** `rwanga-editor/tests/e2e/settings/gap-3-5-shortcut-collisions.spec.js`, 4/4 pass —
      real keypresses in a launched Electron app.
