...
- **HEAD:** `0ed99f65` (S7.1 UI-localisation brief) · **last slice-close:**
  `0ed99f65` · **Branch:** `main` · pushed. *(Update both SHAs at every §0.3 close.)*
...
| ~~GAP-2-5~~ | ~~"Legal" paper size absent from Page Setup~~ **CLOSED 2026-07-29** — and the first read was wrong: Legal was never a dead entry. The layout/pagination engine has always supported it (dims, resolution, and `linesPerPage` asserted by unit tests); only the settings registry's options list omitted it, so Settings hid a working capability. `'legal'` added; proven to produce a real **1344px (14in)** page. Legal is **not** the screenplay industry size (Letter is; A4 elsewhere) — it is simply no longer unreachable | **CLOSED** | `../plans/evidence/S2.4F-paper-size.md` |
...
| GAP-2-1 | ~~Flow view: New doc opens with a dead band above the page~~ **CLOSED 2026-07-29 (S2.3F)** — the oldest outstanding user complaint. Flow started at **227.59px against one page of 1056px** because height was content-driven and no `--page-height` token existed; now it starts at **a minimum of one page** (Letter 1056px / A4 1122.52px), both directions. ⚠ **It is a FLOOR (`min-height` + `height: auto`), not a page size** — Flow stays continuous and unpaginated by design (user, 2026-07-29); page truth lives in Print Preview. Do not hard-pin it | **CLOSED** | `../plans/evidence/S2.3F-new-doc-geometry.md` |
